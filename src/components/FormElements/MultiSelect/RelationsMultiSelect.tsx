...
// Define relationship options with proper values
...
const RelationsMultiSelect: React.FC<DropdownProps> = ({
  id,
  name,
  label = "RELATIONSHIP",
  ...
  // Initialize options
  useEffect(() => {
    const initialOptions = relationshipOptions.map(option => ({ value: option.value, text: option.label, selected: false }));
  ...
  }, [value, options]); // Only depend on value
        ...
        <div className="flex flex-col items-center">
          <div className="relative z-20 inline-block w-full">
            <div className="relative flex flex-col items-center">
              ...
              {/* Dropdown Options */}
              <div
                className={`absolute left-0 top-full z-50 w-full overflow-y-auto rounded bg-white shadow-1 dark:bg-dark-2 dark:shadow-card ${isOpen() ? "block" : "hidden"
                  }`}
                ref={dropdownRef}
              ...
              >
                <div className="flex w-full flex-col">
                  {options.map((option, index) => (
                    <div key={index}>
                      <div
                        className={`w-full cursor-pointer rounded-t border-b border-stroke hover:bg-primary/5 dark:border-dark-3 ${option.selected ? "bg-primary/10" : ""
                          }`}
...
export default RelationsMultiSelect;